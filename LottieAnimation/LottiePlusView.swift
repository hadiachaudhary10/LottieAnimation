...
struct LottiePlusView: UIViewRepresentable {
  ...
  @Binding var fromFrame: CGFloat
  @Binding var toFrame: CGFloat
  init(name: String,
       loopMode: LottieLoopMode = .playOnce,
       animationSpeed: CGFloat = 1,
       contentMode: UIView.ContentMode = .scaleAspectFit,
       play: Binding<Bool> = .constant(true),
       fromFrame: Binding<CGFloat> = .constant(0.0),
       toFrame: Binding<CGFloat> = .constant(0.0)) {
    self.name = name
    ...
    self._play = play
    self._fromFrame = fromFrame
    self._toFrame = toFrame
  }
